...
const CommonWrappableSchema = zod.z.object({
  contractAddress: getContract.AddressSchema
});
...
const ERC721WrappableSchema = CommonWrappableSchema.extend({
  tokenId: getContract.BigNumberishSchema
});
...
const ERC1155WrappableSchema = CommonWrappableSchema.extend({
  tokenId: getContract.BigNumberishSchema,
  quantity: getContract.BigNumberishSchema
});
...
const ERC1155RewardSchema = ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: getContract.BigNumberishSchema
});
...
const ERC20RewardContentsSchema = ERC20RewardSchema.extend({
  totalRewards: getContract.BigNumberishSchema.default("1")
});
...
const PackMetadataInputSchema = PackRewardsOutputSchema.extend({
  ...
  rewardsPerPack: getContract.BigNumberishSchema.default("1"),
  openStartTime: getContract.RawDateSchema.default(new Date())
});
...
class PackVRF {
  constructor(network, address, storage, options, chainId) {
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new getContract.ContractWrapper(network, address, IPackVRFAbi__default["default"], options);
    QueryParams._defineProperty(this, "featureName", getContract.FEATURE_PACK_VRF.name);
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await getContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  getLinkContract() {
    const linkAddress = getContract.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new getContract.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options);
    return new getContract.Erc20(contract, this.storage, this.chainId);
  }
}
 ...
 * @public
 */
class Pack extends getContract.StandardErc1155 {
  /**
   * Configure royalties
   ...
   */
  get vrf() {
    return getContract.assertEnabled(this._vrf, getContract.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    let options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
    let abi = arguments.length > 4 ? arguments[4] : undefined;
    let chainId = arguments.length > 5 ? arguments[5] : undefined;
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new getContract.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        openzeppelin: {
    ...
    this.abi = abi;
    this.erc1155 = new getContract.Erc1155(this.contractWrapper, this.storage, chainId);
    this.metadata = new getContract.ContractMetadata(this.contractWrapper, getContract.PackContractSchema, this.storage);
    this.app = new getContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new getContract.ContractRoles(this.contractWrapper, Pack.contractRoles);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(getContract.getRoleHash("transfer"), ethers.ethers.constants.AddressZero);
  ...
  async createTo(to, metadataWithRewards) {
    const uri = await getContract.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await getContract.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
      ...
      const hasAllowance = await getContract.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, totalQuantity);
      if (!hasAllowance) {
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await getContract.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await getContract.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
  ...
  detectVrf() {
    if (getContract.detectContractFeature(this.contractWrapper, "PackVRF")) {
