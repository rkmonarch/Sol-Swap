 ...
 * @beta
 */
class Multiwrap extends getContract.StandardErc721 {
  /**
   * Configure royalties
  ...
  constructor(network, address, storage) {
    ...
    this.abi = abi;
    this.metadata = new getContract.ContractMetadata(this.contractWrapper, getContract.MultiwrapContractSchema, this.storage);
    this.app = new getContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new getContract.ContractRoles(this.contractWrapper, Multiwrap.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await getContract.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
  ...
  async wrap(contents, wrappedTokenMetadata, recipientAddress) {
    const uri = await getContract.uploadOrExtractURI(wrappedTokenMetadata, this.storage);
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await getContract.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
        const hasAllowance = await getContract.hasERC20Allowance(this.contractWrapper, erc20.contractAddress, normalizedQuantity);
        if (!hasAllowance) {
