...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    this.metadata = new getContract.ContractMetadata(this.contractWrapper, getContract.MarketplaceContractSchema, this.storage);
    this.app = new getContract.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new getContract.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
    ...
    this.direct = new getContract.MarketplaceDirect(this.contractWrapper, this.storage);
    this.auction = new getContract.MarketplaceAuction(this.contractWrapper, this.storage);
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new getContract.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    switch (listing.listingType) {
      case getContract.ListingType.Auction:
        {
          return await this.auction.mapListing(listing);
        }
      case getContract.ListingType.Direct:
        {
  ...
  async getActiveListings(filter) {
    ...
    return filtered.filter(l => {
      return l.type === getContract.ListingType.Auction && ethers.BigNumber.from(l.endTimeInEpochSeconds).gt(now) && ethers.BigNumber.from(l.startTimeInEpochSeconds).lte(now) || l.type === getContract.ListingType.Direct && l.quantity > 0;
    });
  }
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(getContract.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await getContract.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
      ...
      case getContract.ListingType.Direct:
        {
          ...
          return await this.direct.makeOffer(listingId, quantity, getContract.isNativeToken(listing.currency) ? getContract.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
        }
      case getContract.ListingType.Auction:
        {
  ...
  async allowListingFromSpecificAssetOnly(contractAddress) {
    ...
    if (members.includes(ethers.constants.AddressZero)) {
      encoded.push(this.encoder.encode("revokeRole", [getContract.getRoleHash("asset"), ethers.constants.AddressZero]));
    }
    encoded.push(this.encoder.encode("grantRole", [getContract.getRoleHash("asset"), contractAddress]));
  ...
  async allowListingFromAnyAsset() {
    ...
    for (const addr in members) {
      encoded.push(this.encoder.encode("revokeRole", [getContract.getRoleHash("asset"), addr]));
    }
    encoded.push(this.encoder.encode("grantRole", [getContract.getRoleHash("asset"), ethers.constants.AddressZero]));
    await this.contractWrapper.multiCall(encoded);
  }
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof getContract.ListingNotFoundError) {
          return undefined;
        } else {
      ...
      if (listing.type === getContract.ListingType.Auction) {
        return listing;
      }
